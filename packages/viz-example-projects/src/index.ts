...
const createFeatureSeries = (
  ...
  return Array.from({ length }, (_, index) =>
    Number(generator(index).toFixed(4)),
  );
};
...
const exampleAudioPlaceholderBytes = new TextEncoder().encode(
  ...
  featureSeries: [
    {
      ...
      values: createFeatureSeries(
        frameCount,
        (index) => 0.3 + Math.abs(Math.sin(index / 9)) * 0.7,
      ),
    },
    {
      name: 'loudness',
      ...
      values: createFeatureSeries(
        frameCount,
        (index) => 0.2 + Math.abs(Math.sin(index / 16)) * 0.6,
      ),
    },
    {
      name: 'spectral-flux',
      ...
      values: createFeatureSeries(
        frameCount,
        (index) => 0.15 + Math.abs(Math.cos(index / 11)) * 0.5,
      ),
    },
  ],
};

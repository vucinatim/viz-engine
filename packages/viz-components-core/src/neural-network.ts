...
const resolveTriggerEvents = ({
  ...
  let previousTrigger =
    firstRelevantFrame > 0
      ? readSignalSettings(sampleSettings(firstRelevantFrame - 1))
          .trigger
      : false;
  ...
  render: ({
    ...
    return {
      ...
      parameters: {
        ...
        seed: Math.round(
          clamp(asNumber(settings.seed, 42), 1, 10_000),
        ),
        tubeRadius: clamp(
          asNumber(settings.tubeRadius, 0.25),
          0.05,
          1,
        ),
        neuronColor: asString(settings.neuronColor, "#00CED1"),
        somaEmission: asString(
          settings.somaEmission,
          "rgb(255, 138, 201)",
        ),
        emissiveIntensity: clamp(
          asNumber(settings.emissiveIntensity, 2),
          0,
          5,
        ),
        metalness: clamp(
          asNumber(settings.metalness, 0),
          0,
          1,
        ),
        roughness: clamp(
          asNumber(settings.roughness, 0.9),
          0,
          1,
        ),
        fresnelPower: clamp(
          asNumber(settings.fresnelPower, 3),
          0.5,
          8,
        ),
        growth: clamp(asNumber(settings.growth, 1), 0, 1),
        dendriteReach: clamp(
          asNumber(settings.dendriteReach, 20),
          5,
          40,
        ),
        ...
        activationDecay: clamp(
          asNumber(settings.activationDecay, 0.4),
          0,
          10,
        ),
        bloomEnabled: asBoolean(postProcessing.bloom, false),
        bloomStrength: clamp(
          asNumber(postProcessing.bloomStrength, 0.2),
          0,
          3,
        ),
        bloomRadius: clamp(
        ...
        depthOfFieldFocus: clamp(
          asNumber(postProcessing.dofFocus, 10),
          1,
          50,
        ),

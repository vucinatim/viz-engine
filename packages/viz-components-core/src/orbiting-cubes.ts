...
export const orbitingCubesComponent: VizComponentImplementation = {
  ...
  render: ({ frameContext, layer, settings }) => ({
    ...
    parameters: {
      ...
      metalness: Math.min(
        1,
        Math.max(0, asNumber(settings.metalness, 0.95)),
      ),
      roughness: Math.min(
        1,
        Math.max(0, asNumber(settings.roughness, 0.5)),
      ),
      ...
      lightIntensity: Math.max(
        0,
        asNumber(settings.lightIntensity, 500),
      ),
      ...
      rotationSpeed: Math.max(
        0,
        asNumber(settings.rotationSpeed, 0.1),
      ),
    },

...
const resolveShockwaveAges = ({
  ...
  const firstFrame = Math.max(
    0,
    frame - Math.ceil(lifetimeSeconds * fps) - 1,
  );
  let previous =
    firstFrame > 0 ? getTrigger(sampleSettings(firstFrame - 1)) : 0;
  const ages: number[] = [];

  for (let sampledFrame = firstFrame; sampledFrame <= frame; sampledFrame += 1) {
    const current =
      sampledFrame === frame
        ? getTrigger(settings)
  ...
  render: ({
    ...
    return {
      ...
      parameters: {
        ...
        archCount: Math.round(
          clamp(asNumber(structure.archCount, 24), 8, 32),
        ),
        ...
        floorExtent: clamp(
          asNumber(structure.floorExtent, 120),
          40,
          180,
        ),
        ...
        particleDrift: clamp(
          asNumber(motion.particleDrift, 0.7),
          0,
          3,
        ),
        coreRotation: clamp(
          asNumber(motion.coreRotation, 0.65),
          -3,
          3,
        ),
        structurePulse:
        ...
        spectralShimmer:
          ...
          masterResponse,
        onsetResponse:
          clamp(asNumber(reactivity.onsetResponse, 1), 0, 2.5) *
          masterResponse,
        bloomAccent:
        ...
        ambientLevel: clamp(
          asNumber(lighting.ambientLevel, 0.12),
          0,
          2,
        ),
        ...
        bloomStrength: clamp(
          asNumber(lighting.bloomStrength, 0.72),
          0,
          3,
        ),
        bloomRadius: clamp(
          asNumber(lighting.bloomRadius, 0.62),
          0,
          1,
        ),
        bloomThreshold: clamp(
          asNumber(lighting.bloomThreshold, 0.18),
          0,
          1,
        ),
        exposure: clamp(asNumber(lighting.exposure, 1.05), 0.25, 2.5),
        fogDensity: clamp(
          asNumber(lighting.fogDensity, 0.014),
          0,
          0.08,
        ),
      },

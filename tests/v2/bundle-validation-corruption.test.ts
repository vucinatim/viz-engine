...
describe("Viz bundle corruption validation", () => {
  it("reports invalid manifest kind and schema version", () => {
    ...
    try {
      ...
      expect(loaded.issues.map((issue) => issue.code)).toContain("invalid-manifest-kind");
      expect(loaded.issues.map((issue) => issue.code)).toContain("invalid-manifest-schema-version");
    } finally {
  ...
  it("reports missing asset and artifact files", () => {
    ...
    try {
      ...
      unlinkSync(join(bundleDirectory, "baked/artifact-audio-standard-main.json"));

      const loaded = loadLocalVizProjectBundle(bundleDirectory);
      expect(loaded.issues.map((issue) => issue.code)).toContain("missing-asset-file");
      expect(loaded.issues.map((issue) => issue.code)).toContain("missing-artifact-file");
    } finally {
      rmSync(bundleDirectory, { recursive: true, force: true });
    }
  });
  ...
  it("reports orphan bundle entries that are not referenced by the project", () => {
    ...
    try {
      ...
      const loaded = loadLocalVizProjectBundle(bundleDirectory);
      expect(loaded.issues.map((issue) => issue.code)).toContain("orphan-asset-entry");
      expect(loaded.issues.map((issue) => issue.code)).toContain("orphan-artifact-entry");
    } finally {
      rmSync(bundleDirectory, { recursive: true, force: true });
    }
  });
  ...
  it("reports project references that are missing manifest entries", () => {
    ...
    try {
      ...
      const loaded = loadLocalVizProjectBundle(bundleDirectory);
      expect(loaded.issues.map((issue) => issue.code)).toContain("missing-asset-entry");
      expect(loaded.issues.map((issue) => issue.code)).toContain("missing-artifact-entry");
    } finally {
      rmSync(bundleDirectory, { recursive: true, force: true });
    }
  });

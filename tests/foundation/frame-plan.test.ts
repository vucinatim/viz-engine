...
describe("Viz frame planning", () => {
  it("resolves artifact-backed inputs into a deterministic layer frame plan", () => {
    ...
    const coverLayer = framePlan.layers.find((layer) => layer.layerId === "layer-cover");
    ...
    expect(
      typeof (coverLayer?.resolvedInputs.image.value as { imageSourceUri?: unknown } | undefined)
        ?.imageSourceUri,
    ).toBe("string");
    ...
    expect(reactiveBarsLayer?.resolvedInputs.bass.sourceKind).toBe("graph-output");
  ...
  it("executes components into a deterministic render plan", () => {
    ...
    expect(renderPlan.layers.every((layer) => layer.node !== undefined && layer.node !== null)).toBe(true);

    const coverLayer = renderPlan.layers.find((layer) => layer.layerId === "layer-cover");
    const barsLayer = renderPlan.layers.find((layer) => layer.layerId === "layer-bars");

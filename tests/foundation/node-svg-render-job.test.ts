...
describe("Node SVG render jobs", () => {
  it("materializes deterministic still and contact-sheet artifacts", async () => {
    const outputDirectory = mkdtempSync(
      join(tmpdir(), "viz-node-svg-render-"),
    );
    ...
    const stillMarkup = readFileSync(
      fileURLToPath(stillOutput!.uri),
      "utf8",
    );
    ...
    const contact = await service.wait(
      service.start(contactRequest).id,
    );

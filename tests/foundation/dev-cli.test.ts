...
describe('Viz local-first CLI surface', () => {
  ...
  it('returns consistent structured command and argument errors', async () => {
    await expect(runVizCli(['unknown'])).resolves.toEqual({
      ok: false,
      command: 'unknown',
      payload: {
        code: 'unknown-command',
        message: 'Unknown command scope: unknown.',
      },
    });
    await expect(
      runVizCli(['bundle', 'frame', '--dir']),
  ...
  it('runs a headless bundle contact-sheet job through the CLI', async () => {
    const tempDirectory = mkdtempSync(join(tmpdir(), 'viz-cli-render-job-'));
    ...
    try {
      ...
      expect(
        readFileSync(new URL(payload.job.result.outputs[0]!.uri), 'utf8'),
  ...
  it('exports the in-memory example project into a portable bundle', () => {
    const tempDirectory = mkdtempSync(
      join(tmpdir(), 'viz-example-bundle-export-'),
    );
  ...
  it('roundtrips a loaded portable bundle through export and reload', () => {
    ...
    try {
      const output = exportBundleProject(
      ...
      expect(svgOutput.ok).toBe(true);
      expect((svgOutput.payload as { svg: string }).svg).toContain(
        'data-layer-id="layer-bars"',
      );
    } finally {
      rmSync(tempDirectory, { recursive: true, force: true });
    }
  ...
  it('roundtrips a native model asset through the portable bundle', () => {
    ...
    try {
      ...
      expect(readFileSync(join(tempDirectory, modelEntry.path))).toEqual(
  ...
  it('applies canonical project actions to the example project and exports the result', () => {
    const tempDirectory = mkdtempSync(
      join(tmpdir(), 'viz-example-action-apply-'),
    );
    ...
    try {
      ...
      expect(reloaded.issues).toHaveLength(0);
      expect(
        reloaded.project.layers.find((layer) => layer.id === 'layer-background')
          ?.settings,
      ).toMatchObject({
        color: '#101e2d',
      });
  ...
  it('applies canonical project actions to a portable bundle and keeps it renderable', () => {
    const tempDirectory = mkdtempSync(
      join(tmpdir(), 'viz-bundle-action-apply-'),
    );

...
describe('portable execution manifests', () => {
  it('validates the pinned Signal Cathedral execution environment and content', () => {
    const loaded = loadLocalVizProjectBundle(signalCathedralBundle);
    ...
    expect(loaded.executionManifest?.nodePackages[0]?.nodeTypes).toEqual([
      'add',
      'clamp',
      'graph-input',
      'multiply',
  ...
  it('detects changed bundle bytes and missing node implementation coverage', () => {
    ...
    try {
      const manifestPath = join(bundleDirectory, 'execution-manifest.json');
      ...
      manifest.assets[0]!.contentIdentity = `sha256:${'0'.repeat(64)}`;
      ...
      expect(codes).toContain('execution-content-identity-mismatch');

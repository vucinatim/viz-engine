              ...
              <div
                ...
                <div className="rounded-xl border border-white/10 bg-black/20 p-3">
                  <div className="mb-2 flex items-center justify-between text-xs text-white/55">
                    <span>Inputs</span>
                    <span>{component.inputCount}</span>
                  </div>
          ...
          <div className="space-y-3">
            <div className="rounded-2xl border border-white/10 bg-white/[0.03] p-4">
              ...
              <div className="mt-3 grid gap-2 text-xs text-white/65">
                <div className="flex items-center justify-between rounded-xl border border-white/10 bg-black/20 px-3 py-2">
                  <span>Audio input mode</span>
                  <span>{snapshot.snapshot.audioDiagnostics.inputMode}</span>
                </div>
                <div className="flex items-center justify-between rounded-xl border border-white/10 bg-black/20 px-3 py-2">
                  <span>Live audio</span>
                  <span>
                    {snapshot.snapshot.audioDiagnostics.usesLiveAudio
                      ? 'enabled'
                      : 'inactive'}
                  </span>
                </div>
                <div className="flex items-center justify-between rounded-xl border border-white/10 bg-black/20 px-3 py-2">
                  <span>Baked artifacts</span>
                  <span>
                    {snapshot.snapshot.audioDiagnostics.usesBakedArtifacts
                      ? 'available'
                      : 'missing'}
                  </span>
                </div>
                <div className="flex items-center justify-between rounded-xl border border-white/10 bg-black/20 px-3 py-2">
                  <span>Resources</span>
                  <span>
                    {snapshot.snapshot.resourceCounts.assets} assets ·{' '}
                    {snapshot.snapshot.resourceCounts.artifacts} artifacts
                  </span>
                </div>
              </div>
            </div>
            ...
            <div className="rounded-2xl border border-white/10 bg-white/[0.03] p-4">
              ...
              {groupedIssues.length > 0 ? (
                ...
                <div className="rounded-xl border border-emerald-500/20 bg-emerald-500/10 px-3 py-3 text-xs text-emerald-100">
                  No surfaced issues at the current frame.
                </div>
              )}
            </div>
            ...
            <div className="rounded-2xl border border-white/10 bg-white/[0.03] p-4">
              ...
              <pre className="max-h-64 overflow-auto rounded-xl border border-white/10 bg-black/20 p-3 text-[11px] text-white/70">
                {JSON.stringify(snapshot.snapshot.session.workingProject, null, 2)}
              </pre>
            </div>
          </div>
        )}

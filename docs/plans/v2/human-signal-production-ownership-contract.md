# Human Signal production ownership

EC-02 establishes `@viz-engine/production-human-signal` as the canonical owner
of the project factory, approved score and graph requirements, resource recipes,
trusted execution environment, bundle entrypoint, and package-local tests.
Historical Gate 1 generators retain ownership of their immutable proposal
artifacts. Runtime, component, example, CLI and Studio packages must not import
or recognize Human Signal.

## Observable contract

The factory creates independent canonical project documents. Its initial
composition has nine layer records with the treatment's baseline settings and
static initial layer selection. This is an ownership foundation, not the
full-window skeleton. There are zero implemented graphs. Seven graph
requirements remain authoring data, never empty executable graphs.

The generic bundle writer and execution manifest own executable identities.
The production entrypoint verifies the approved source music, four model files,
treatment, musical map, historical capability catalog and Gate 1 packet before
...
Evidence lanes: project-semantic and code-architecture, on the exact claimed
